...
export function LocalImportDialog({
	...
	// ─── Use native <dialog> showModal() — places dialog in the top layer ───────
	...
	return (
		<>
			...
			<dialog
				...
				className="m-auto w-full max-w-md rounded-xl border border-border bg-card p-0 shadow-2xl backdrop:bg-black/50 backdrop:backdrop-blur-sm open:animate-[dialog-in_0.2s_ease-out]"
				aria-modal="true"
			...
			>
				<div className="p-5">
					...
					<p className="mb-4 text-xs text-muted-foreground">{ui.subtitle}</p>
						...
						<div className="space-y-4">
							<div className="space-y-1.5">
								...
								<select
									...
									onChange={(e) =>
										setSelectedProvider(e.target.value as LocalImportProvider)
									}
								...
								>
									{(Object.keys(PROVIDER_LABELS) as LocalImportProvider[]).map(
										(slug) => (
								...
								<div className="rounded-lg border border-border bg-muted/30 p-3 text-xs">
									<div className="space-y-1">
										<p>
											...
											<span className="font-medium">{detected.identifier}</span>
										</p>
										{detected.planName && (
											<p>
												<span className="text-muted-foreground">{ui.plan}</span>{" "}
